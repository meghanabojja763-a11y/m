...
The app will:
- Enhance and morphologically process all images  
- Compare the single image with each image in the folder  
- Detect if the image is present based on similarity
""")
...
def preprocess_image(image_bytes):
    """Read, resize, enhance, and apply morphology to an image."""
...
if query_file and folder_zip:
    with st.spinner("Processing images..."):
        ...
        # Extract ZIP folder to a temp directory
        temp_dir = tempfile.mkdtemp()
        with zipfile.ZipFile(folder_zip, "r") as zip_ref:
            zip_ref.extractall(temp_dir)

        matched_files = []
        threshold = 0.85  # More tolerant threshold

        # Compare each image in folder
        ...
        if matched_files:
            ...
            for name, score in matched_files:
                ...
                st.image(os.path.join(temp_dir, name), caption=f"{name} (Score: {score:.4f})", use_container_width=True)
        else:
